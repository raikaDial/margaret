...
void BNO055::get_calibration(BNO055_CALIBRATION_TypeDef *calib)
{
    select_page(0);
    dt[0] = ACCEL_OFFSET_X_LSB;
    _i2c.write(chip_addr, dt, 1, true);
    _i2c.read(chip_addr, dt, 6, false);
    
    for(int i=0; i<6; ++i) {
        calib -> acc_offset[i] = dt[i];
    }
    
    dt[0] = MAG_OFFSET_X_LSB;
    _i2c.write(chip_addr, dt, 1, true);
    _i2c.read(chip_addr, dt, 6, false);
    
    for(int i=0; i<6; ++i) {
        calib -> mag_offset[i] = dt[i];
    }
        
    dt[0] = GYRO_OFFSET_X_LSB;
    _i2c.write(chip_addr, dt, 1, true);
    _i2c.read(chip_addr, dt, 6, false);
    
    for(int i=0; i<6; ++i) {
        calib -> gyro_offset[i] = dt[i];
    }
        
    dt[0] = ACCEL_RADIUS_LSB;
    _i2c.write(chip_addr, dt, 1, true);
    _i2c.read(chip_addr, dt, 2, false);
    
    for(int i=0; i<2; ++i) {
        calib -> acc_radius[i] = dt[i];
    }
    ...
    for(int i=0; i<2; ++i) {
        calib -> mag_radius[i] = dt[i];
    }
}

void BNO055::set_calibration(BNO055_CALIBRATION_TypeDef *calib) {
    change_fusion_mode(CONFIGMODE);
    
    select_page(0);
    dt[0] = ACCEL_OFFSET_X_LSB;
    for(int i=0; i<6; ++i) {
        dt[i+1] = calib -> acc_offset[i];
    }
    _i2c.write(chip_addr, dt, 7, false);
    
    dt[0] = MAG_OFFSET_X_LSB;
    for(int i=0; i<6; ++i) {
        dt[i+1] = calib -> mag_offset[i];
    }
    _i2c.write(chip_addr, dt, 7, false);
    
    dt[0] = GYRO_OFFSET_X_LSB;
    for(int i=0; i<6; ++i) {
        dt[i+1] = calib -> gyro_offset[i];
    }
    _i2c.write(chip_addr, dt, 7, false);    
 
    dt[0] = ACCEL_RADIUS_LSB;
    for(int i=0; i<2; ++i) {
        dt[i+1] = calib -> acc_radius[i];
    }
    _i2c.write(chip_addr, dt, 3, false);    
    
    dt[0] = MAG_RADIUS_LSB;
    for(int i=0; i<2; ++i) {
        dt[i+1] = calib -> mag_radius[i];
    }
    _i2c.write(chip_addr, dt, 3, false);      
...
void BNO055::unit_selection(void)
{
    select_page(0);
    dt[0] = BNO055_UNIT_SEL;
    dt[1] = UNIT_ORI_ANDROID + UNIT_ACC_MSS + UNIT_GYR_RPS + UNIT_EULER_DEG + UNIT_TEMP_C;
    _i2c.write(chip_addr, dt, 2, false);
}
...
void BNO055::change_fusion_mode(uint8_t mode)
{
    ...
    switch (mode) {
        ...
        case MODE_NDOF:
            if (current_mode != CONFIGMODE) {   // Can we change the mode directly?
                dt[0] = BNO055_OPR_MODE;
                dt[1] = CONFIGMODE;
                _i2c.write(chip_addr, dt, 2, false);
                wait_ms(19);    // wait 19mS
