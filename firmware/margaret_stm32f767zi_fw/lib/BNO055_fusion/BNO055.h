...
public:
    ...
    */
     ...
     * @param uint8_t type of data address
     */
    void get_calibration(BNO055_CALIBRATION_TypeDef *calib);

    void set_calibration(BNO055_CALIBRATION_TypeDef *calib);
...
//---------------------------------------------------------
#if 0
static Serial pc(USBTX, USBRX, 921600);
static BNO055_GRAVITY_TypeDef gravity;
static BNO055_CALIBRATION_TypeDef calib;
// Calibration
//  Please refer BNO055 Data sheet 3.10 Calibration & 3.6.4 Sensor calibration data
static void bno055_calibration(BNO055 & imu){
    uint8_t d;
    ...
    while (true){
        ...
        pc.printf("Calb dat = 0x%x target  = 0x%x\r\n", (d & (0x3 << 4)), 0x3 << 4);
        if ((d & (0x3 << 4)) == (0x3 << 4)) {
            break;
        }
        wait(1.0);
    ...
    if(current_mode != MODE_IMU) {
        ...
        pc.printf("Magnetometer is disabled in current mode, skipping calibration\r\n");
    }
    //---------- Accelerometer Caliblation ---------------------------------------------------------
    ...
    while (true){
        ...
        pc.printf("Calb dat = 0x%x target  = 0x%x ACC:X %3.0f, Y %3.0f, Z %3.0f\r\n",
                   (d & (0x3 << 2)), 0x3 << 2, gravity.x, gravity.y, gravity.z);
        if ((d & (0x3 << 2)) == (0x3 << 2)){break;}
        if (pc.readable()){break;}
        wait(1.0);
    }
    d = imu.read_calib_status();
    if( ((current_mode != MODE_IMU) && (d == 0xff))
        || ((current_mode == MODE_IMU) && ( (d & ((0x3 << 4) | (0x3 << 2))) == ((0x3 << 4) | (0x3 << 2))) )
    ) {
        pc.printf("-> All of Calibration steps are done successfully!\r\n\r\n");
        wait_ms(50);
        imu.get_calibration(&calib);
        pc.printf("ACCEL OFFSET: ");
        for(int i=0; i<6; ++i) {
            pc.printf("0x%x ", calib.acc_offset[i]);
        }
        pc.printf("\r\nMAG OFFSET: ");
        for(int i=0; i<6; ++i) {
            pc.printf("0x%x ", calib.mag_offset[i]);
        }
        pc.printf("\r\nGYRO OFFSET: ");
        for(int i=0; i<6; ++i) {
            pc.printf("0x%x ", calib.gyro_offset[i]);
        }
        pc.printf("\r\nACCEL RADIUS: ");
        for(int i=0; i<2; ++i) {
            pc.printf("0x%x ", calib.acc_radius[i]);
        }
        pc.printf("\r\nMAG RADIUS: ");
        for(int i=0; i<2; ++i) {
            pc.printf("0x%x ", calib.mag_radius[i]);
        }
